...
class _AdminSettingsScreenState extends State<AdminSettingsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        client.from('profiles').select('id, email, role, is_active').order('email'),
        client.from('parameter_configurations').select().eq('id', 1).maybeSingle(),
      ]);
  ...
  Future<void> _deactivateUser(Map<String, dynamic> user) async {
    ...
    try {
      await client.from('profiles').update({'is_active': false}).eq('id', user['id']);
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if (!isAdmin || _loading)
                Center(child: Text(_error ?? 'Loading...', style: AppTextStyles.body))
              else if (_error != null)
  ...
  Widget _buildConfiguration() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('pH/EC Parameter Configuration', style: AppTextStyles.sectionTitle),
        ...
        children: [
          ...
          for (final user in _users)
            ListTile(
              ...
              subtitle: Text((user['is_active'] as bool? ?? false) ? 'Active' : 'Revoked'),
              trailing: Wrap(
                spacing: 8,
